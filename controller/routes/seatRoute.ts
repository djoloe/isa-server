...
router.post('/seat', async function(req,res,next) {
    const saved = await seatService.parseSeats(req.body);
    res.send(saved);
})

router.post('/loadSeatsFlight', async function(req,res,next) {
    const divs = await seatService.checkFlightDivDB(req.body.idFlight);
    res.send(divs);
})

router.post('/removeReservation', async function (req,res,next) {
